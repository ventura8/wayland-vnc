...
def main() -> int:
    ...
    from wayland_vnc import runtime, settings, settings_app
    ...
    if application is None:
            ...
            return sorted(
                {
                    type(w).__name__
                    for w in walk(root)
                    if text_dir(w) != rtl_dir and not inside_spin(w)
                }
            )
        ...
        check(
            os.environ.get("LANGUAGE") == "ro", "the toolkit's own strings are pointed at Romanian"
        )
