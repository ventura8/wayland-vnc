...
build_flatpak() {
  flatpak-builder --force-clean --repo=packaging/flatpak/repo \
...
build_snap() {
  local snap="packaging/snap/wayland-vnc_${version}.snap"
  # Only the snap this run produces is published: a `find` for *.snap also picked up
  # any earlier build left under packaging/snap.
  rm -f -- "$snap"
  (cd packaging/snap && snapcraft --output "wayland-vnc_${version}.snap")
  cp -- "$snap" "$out/"
}
