...
exists. Two source packages go to the PPA: `wayland-vnc`, and the GNOME backend
`wayland-vnc-grd`, which `scripts/prepare-grd-ppa-source.sh` assembles with the
checksum-pinned GNOME Remote Desktop and LibVNCServer tarballs (`sources.json`) vendored
into the tree, because Launchpad builds offline; its `debian/rules`
(`packaging/grd/debian/`) compiles both with the project's patch series into
`/opt/wayland-vnc`. Both carry the same PPA version and changelog line.
